...
func init() { register(repo{}) }

func (_ repo) Name() string { return "repo" }
func (_ repo) Desc() string { return `Explains what is "repository"` }
func (_ repo) Help() {
...
- ssh
    ...
    2. Setup public key autheticating on remote, allowing passwordless login.
    3. Install moobk on remote user, add it to PATH envvar.
    4. Prepare local ssh environment to connect to remote without any prompy (eg: add
    ...
    You may use query string to tell ssh correct key file and bypass host key
    verification (DANGER!). Or, just setup ssh env for root.
...
func (h repo) Exec(args []string) (ret int) {
	h.Help()
	return
}
